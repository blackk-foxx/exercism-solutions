...
song = intercalate "\n" verses
  ...
  verses = [first_verse] ++ middle_verses ++ [last_verse]
  first_verse = head verse_heads ++ verse_ending
  middle_verses = [a ++ b ++ c ++ verse_ending | (a,b,c) <- zip3 middle_verse_heads (init verse_middles) verse_tails]
  middle_verse_heads = init (tail verse_heads)
  ...
  verse_tail_parts = ["She swallowed the " ++ p ++ "\n" | p <- make_verse_tail_unique_parts animals]
