...
song = intercalate "\n" verses
  ...
  make_verse_tails [] = []
  make_verse_tails (a) = make_verse_tails (init a) ++ [intercalate "" (reverse a)]
  ...
  make_verse_tail_unique_parts [x] = []
  make_verse_tail_unique_parts (x:xs) = [part] ++ (make_verse_tail_unique_parts xs)
    where
    part = (head xs) ++ " to catch the " ++ x ++ extra x ++ "."
    extra "spider" = " that wriggled and jiggled and tickled inside her" 
    extra _ = ""