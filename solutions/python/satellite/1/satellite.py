...
def tree_from_traversals(preorder: list[str], inorder: list[str]) -> dict:
    ...
    root = preorder[0]
    if root == inorder[-1]:
        left = tree_from_traversals(preorder[1:], inorder[:-1])
        right = {}
    elif root == inorder[0]:
        left = {}
        right = tree_from_traversals(preorder[1:], inorder[1:])
    else:
...
def validate(preorder: list[str], inorder: list[str]) -> None:
    """
        Validate the given pre-order and in-order traversals.
    """
