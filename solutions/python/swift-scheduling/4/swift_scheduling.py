...
def delivery_date(start: datetime, description: str) -> datetime:
    """
        Apply the given description to the given start datetime.
    """
...
def friday_after(date: datetime) -> datetime:
    """Return the first Friday after the given date."""
    return date + timedelta(days = FRIDAY - date.weekday())


def sunday_after(date: datetime) -> datetime:
    """Return the first Sunday after the given date."""
...
def apply_variable_description(start_date: datetime, description: str) -> datetime:
    """Apply the given variable description to the given start_date."""
    if description.endswith("M"):
        target_month = int(description[:-1])
        return first_workday_in_following_month(start_date, target_month)
    if description.startswith("Q"):
        target_quarter = int(description[1:])
        return last_workday_in_following_quarter(start_date, target_quarter)
    raise ValueError(f"Unknown description {description}")


def first_workday_in_following_month(start_date: int, target_month: int) -> datetime:
    """Return the first workday in the given month following the given start_date."""
    target_year = start_date.year if start_date.month < target_month else (start_date.year + 1)
    return first_workday_of(year=target_year, month=target_month)
    

def last_workday_in_following_quarter(start_date: int, target_quarter: int) -> datetime:
    """Return the last workday in the given target_quarter following the given start_date."""
    ...
    return last_workday_before(start_of_quarter_after(target_year, target_quarter))


def first_workday_of(year: int, month: int) -> datetime:
    """Return the first workday in the given year and month."""
...
def last_workday_before(date: datetime) -> datetime:
    """Return the last workday before the given date."""
...
def start_of_quarter_after(year: int, quarter: int) -> datetime:
    """Return the start date of the quarter following the given year and quarter."""
...
def first_day_of(year: int, month: int) -> datetime:
    """Return the start date of the given year and month."""
