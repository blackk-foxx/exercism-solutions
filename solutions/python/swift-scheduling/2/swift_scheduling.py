...
def apply_variable_description(start_date: datetime, description: str) -> datetime:
    if description.endswith('M'):
        ...
        return apply_month_wise_description(start_date, target_month)
    elif description.startswith('Q'):
        target_quarter = int(description[1:])
        return apply_quarter_wise_description(start_date, target_quarter)
    else:
        raise ValueError(f"Unknown description {description}")


def apply_month_wise_description(start_date: int, target_month: int) -> datetime:
...
def apply_quarter_wise_description(start_date: int, target_quarter: int) -> datetime:
