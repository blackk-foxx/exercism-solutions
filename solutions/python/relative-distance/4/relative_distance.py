...
class PathNode:
    ...
    def path_length(self):
        if self.parent:
            return 1 + self.parent.path_length()
        return 0
...
class RelativeDistance:
    ...
    def shortest_path(self, person_a, person_b):
        nodes = deque()
        explored = {person_a}
        nodes.append(PathNode(person_a))
        while len(nodes) > 0:
            next_node = nodes.popleft()
            if next_node.name == person_b:
                return next_node
            for edge in self.nodes[next_node.name].edges:
                if edge not in explored:
                    explored.add(edge)
                    nodes.append(PathNode(name=edge, parent=next_node))
        return None
