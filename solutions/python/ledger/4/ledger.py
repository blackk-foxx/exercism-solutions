...
def format_entries(currency, locale, entries):
    formatter = Formatter(get_symbol(currency), locale)
...
class Formatter:
    ...
    def __init__(self, symbol, locale):
        self.symbol = symbol
        if locale == 'en_US':
    ...
    def format_header_row(*headings):
        return ' | '.join(heading.ljust(width) 
                          for heading, width in zip(headings, COLUMN_WIDTHS))

    def format_entry_row(self, entry):
        return ' | '.join((
            self.format_date(entry.date).ljust(COLUMN_WIDTHS[0]),
            self.format_description(entry.description).ljust(COLUMN_WIDTHS[1]),
            self.format_change(entry.change).rjust(COLUMN_WIDTHS[2]),
        ))

    @staticmethod
    def format_description(description):
        if len(description) > COLUMN_WIDTHS[1]:
            return description[:COLUMN_WIDTHS[1]-3] + '...'
        return description
