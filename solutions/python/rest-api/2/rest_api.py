...
class RestAPI:
    ...
    def post(self, url: str, payload: Optional[str] = None) -> str:
        ...
        match url:
            ...
            case "/iou":
                ...
                return json.dumps({
                    'users': self._get_user_data(sorted([lender, borrower]))
                })
            case _:
                raise ValueError
    ...
    def _get_user_data(self, users: list[str]) -> list[dict]:
        return [{
            ...
            for user, owes, owed_by in [
                (user, *self._get_single_user_debt_data(user))
                for user in users
            ]
        ]

    def _get_single_user_debt_data(self, user: str) -> dict:
        owes = {}
        owed_by = {}
...
class IouTracker:
    ...
    def get_user_balance(self, name: str) -> float:
        return -sum(self._users[name].values())
