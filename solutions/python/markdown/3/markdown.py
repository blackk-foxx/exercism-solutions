...
MAX_HEADING_LEVEL = 6
LIST_ITEM_PATTERN = r'\* (.*)'
HEADING_PATTERN = rf'(#{{1,{MAX_HEADING_LEVEL}}}) (.*)'
...
def format_paragraph(line):
    if line:
        return make_element('p', apply_emphasis(line))
    return ""
...
def parse(markdown):
    ...
    for line in markdown.split('\n') + [""]:
        if _match := re.match(LIST_ITEM_PATTERN, line):
            ...
            if list_items:
                result.append(format_list(list_items))
                list_items = []
            if _match := re.match(HEADING_PATTERN, line):
                result.append(format_heading(_match))
            else:
