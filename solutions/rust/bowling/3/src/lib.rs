...
impl BowlingGame {
        ...
        if self.frame_count() < 10 {
            ...
            let result = 
                (0..10)
                    ...
                    .map(|(frame, roll_count)| self.sum_of_rolls(frame, 0..roll_count))
                    .sum();
            Some(result)
        }
    }
    ...
    fn last_roll_is_valid(&self) -> bool {
        ...
        if self.current_frame_is_final() {
            if self.sum_of_rolls(frame, 0..=1) == 20 {
                if self.sum_of_rolls(frame, 2..=2) > 10 {
                    return false;
                }
            }
            else if self.sum_of_rolls(frame, 0..=0) == 10 {
                if self.sum_of_rolls(frame, 1..=2) > 10 {
                    return false;
                }
            }
        }
        else if self.last_roll_ends_current_frame() && 
