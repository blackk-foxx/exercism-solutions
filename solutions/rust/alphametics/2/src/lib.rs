...
pub fn solve(input: &str) -> Option<HashMap<char, u8>> {
    ...
    for perm in digits.into_iter().permutations(letters.len()) {
        ...
        if can_solve(&mapping, &addends, &symbolic_sum) {
            return Some(mapping);
        }
    }
    None
...
fn create_mapping(letters: &HashSet<char>, digits: &Vec<u8>) -> HashMap<char, u8> {
    zip(letters.iter(), digits).map(|(&c, &n)| (c, n)).collect()
}

fn can_solve(mapping: &HashMap<char, u8>, addends: &Vec<&str>, symbolic_sum: &str) -> bool {
...
fn mapping_is_valid(mapping: &HashMap<char, u8>, addends: &Vec<&str>, symbolic_sum: &str) -> bool {
    is_valid(mapping, symbolic_sum) &&
    addends
        .iter()
        .all(|&a| is_valid(mapping, a))
}
...
fn is_valid(mapping: &HashMap<char, u8>, term: &str) -> bool {
    term.chars().next().is_some_and(|c| mapping.get(&c) != Some(&0))
}