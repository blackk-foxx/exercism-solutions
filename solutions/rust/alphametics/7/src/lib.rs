...
fn mapping_resolves(mapping: &HashMap<char, u8>, addends: &Vec<&str>, symbolic_sum: &str) -> bool {
    if !mapping_is_valid(mapping, addends, symbolic_sum) {
        return false;
    }
    make_numeric(mapping, symbolic_sum) == addends
        .iter()
        .fold(0, |acc, &a| acc + make_numeric(mapping, a))
}

fn mapping_is_valid(mapping: &HashMap<char, u8>, addends: &Vec<&str>, symbolic_sum: &str) -> bool {
    let any_term_starts_with_zero = get_key_for_value(mapping, 0).is_some_and(|zero_mapped_char| 
        symbolic_sum.starts_with(zero_mapped_char) || addends
            .iter()
            .any(|&a| a.starts_with(zero_mapped_char))
    );
    !any_term_starts_with_zero
}
