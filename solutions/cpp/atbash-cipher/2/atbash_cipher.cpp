...
std::string chunked(std::string_view text) {
    ...
    size_t i;
    for (i = 0; i + chunk_size < text.size(); i += chunk_size) {
        result << text.substr(i, chunk_size) << ' ';
    }
    result << text.substr(i, chunk_size);
    return result.str();
}
...
std::string encoded(std::string_view text) {
    std::string result;
    std::transform(text.cbegin(), text.cend(), std::back_inserter(result), [](char c) {return encode_char(tolower(c));});
    return result;
}
