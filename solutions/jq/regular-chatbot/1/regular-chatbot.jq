...
def check_phone_number:
  . as $input | test("\\(\\+\\d\\d\\) \\d\\d\\d-\\d\\d\\d-\\d\\d\\d") 
    | if . then "Thanks! Your phone number is OK." 
      else "Oops, it seems like I can't reach out to \($input)." end
;
...
def nice_to_meet_you:
  . | match("my name is ([\\w-]+)"; "i") | "Nice to meet you, \(.captures[0].string)"
;
