public class SecurityPassMaker
{
    ...
    {
        var result = "Too Important for a Security Pass";
        if (support is Staff)
        {
            result = support.Title;
            if (support is Security security && IsPriorityPersonnel(security))
            {
    ...
    bool IsPriorityPersonnel(Security obj)
    {
        return !(obj is SecurityJunior || obj is SecurityIntern || obj is PoliceLiaison);
    }
}
