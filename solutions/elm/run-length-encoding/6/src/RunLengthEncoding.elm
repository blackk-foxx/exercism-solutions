    ...
    in
        case uncons string of
            ...
            Just(c, xs) -> case countAtStart c xs of
                0 -> encode xs |> cons c
                n -> (fromInt (n + 1)) ++ (xs |> dropLeft n |> encode |> cons c)
...
decode string = 
    let
        takeDigits str = case uncons str of
            Nothing -> ""
            Just(c, xs) -> 
                if isDigit c then takeDigits xs |> cons c
                else ""
        repeat count char = case count of 
            0 -> ""
            n -> repeat (n-1) char |> cons char
        splitDigitsFromString str = 
            let digits = takeDigits str
            in (digits, dropLeft (length digits) str)
